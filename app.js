...
if(__p.get('demo')==='payment'){state.screen='payment';state.base=haircuts[1];state.entry='haircut';state.selectedExtras={'wash-dry':1,'fade':1};state.selectedSource='yandex';state.master=1;state.masterBackendId=101;state.rating=5;state.phone='9991234567';state.receiptPhone='+7 (999) 123-45-67';}
...
function promos(){const selected=state.promotionSelection;return `<div class="promotions-screen">
  ...
  <div class="promotions-grid">
    <button class="promotion-card active-promo ${selected==='senior'?'selected':''}" data-promotion="senior" aria-pressed="${selected==='senior'}">
      <div class="promotion-art"><img src="./brand/action-senior.jpeg" alt="Пенсионер"></div>
    ...
    <button class="promotion-card active-promo ${selected==='father-son'?'selected':''}" data-promotion="father-son" aria-pressed="${selected==='father-son'}">
      <div class="promotion-art"><img src="./brand/action-family.jpeg" alt="Отец и сын"></div>
      <div class="promotion-content">
        <span class="promotion-badge active"><i>✓</i> АКЦИЯ АКТИВНА</span>
        <h2>ОТЕЦ И СЫН</h2>
...
'yandex':`<svg viewBox="0 0 64 64" class="source-svg" aria-hidden="true"><path fill="#e30613" d="M32 8c-10.5 0-19 8.5-19 19 0 14.4 19 29 19 29s19-14.6 19-29c0-10.5-8.5-19-19-19Z"/><circle cx="32" cy="27" r="7.2" fill="#fff"/></svg>`,
'2gis':`<div class="source-badge source-badge-2gis"><span>2GIS</span><i></i></div>`,
'google':`<svg viewBox="0 0 64 64" class="source-svg" aria-hidden="true"><path fill="#34a853" d="M32 9c-10.5 0-19 8.5-19 19 0 14.4 19 29 19 29s19-14.6 19-29c0-10.5-8.5-19-19-19Z"/><path fill="#4285f4" d="M32 9c7.4 0 13.8 4.2 17 10.3L32 32Z" opacity=".96"/><path fill="#fbbc05" d="M49 19.3c1.3 2.4 2 5 2 7.7 0 10.5-8.5 19-19 19V32Z" opacity=".98"/><path fill="#ea4335" d="M32 9 15 19.2A18.9 18.9 0 0 1 32 9Z"/><circle cx="32" cy="27" r="7.2" fill="#fff"/></svg>`,
...
'social':`<div class="source-badge source-badge-vk"><span>VK</span><i>❤</i></div>`,
...
return `<div class="payment-screen">
  ...
  <div class="payment-right">
    ...
    <div class="receipt-delivery">
      ...
      <p>Отправим чек на номер телефона или e-mail.</p>
      <label class="receipt-choice ${state.receiptTo==='phone'?'selected':''}">
        <div class="receipt-choice-main">
          <span class="receipt-choice-icon phone"></span>
          <span class="receipt-choice-copy"><b>Отправить на номер телефона</b><small>${receiptPhoneDisplay()}</small></span>
        </div>
        <button type="button" class="receipt-radio ${state.receiptTo==='phone'?'selected':''}" data-receipt="phone" aria-label="Выбрать телефон"></button>
      </label>
      <div class="receipt-or">ИЛИ</div>
      <label class="receipt-email-row ${state.receiptTo==='email'?'selected':''}">
        <div class="receipt-choice-main">
          <span class="receipt-choice-icon mail"></span>
          <span class="receipt-choice-copy"><b>Отправить на e-mail</b></span>
        </div>
        <input id="receiptEmail" value="${state.receiptEmail}" placeholder="Введите e-mail">
        <button type="button" class="receipt-radio ${state.receiptTo==='email'?'selected':''}" data-receipt="email" aria-label="Выбрать e-mail"></button>
      </label>
      <div class="receipt-help"><span>i</span><small>Мы сохраним e-mail в вашем профиле для отправки чеков в будущем.</small></div>
    </div>
  </div>
</div>`}
function success(){return `<div class="success-screen">
  ...
  <section class="success-qr-grid">
    <article class="success-qr-card">
      ...
      <div class="qr-note"><span class="info-circle">i</span><small>Чек также отправлен на<br>номер телефона или e-mail</small></div>
    </article>
    <article class="success-qr-card">
...
let body='';
if(state.modal==='policy')body=`<h2>ПОЛЬЗОВАТЕЛЬСКОЕ СОГЛАШЕНИЕ</h2><div class="legal"><p>Нажимая «Согласен», вы даёте согласие на обработку номера телефона для участия в бонусной программе, получения информации о балансе и истории посещений.</p><p>Данные используются только для оказания услуг и не передаются третьим лицам без законного основания. Согласие можно отозвать, обратившись к администратору.</p></div><button class="modal-action" data-action="close-modal">СОГЛАСЕН</button>`;
...
return `<div class="overlay"><div class="modal"><button class="modal-close" data-action="close-modal">×</button>${body}</div></div>`
}
...
app.addEventListener('input',e=>{if(e.target.id==='promoCode')state.promoCode=e.target.value.toUpperCase();if(e.target.id==='receiptPhone')state.receiptPhone=e.target.value;if(e.target.id==='receiptEmail')state.receiptEmail=e.target.value;});
fit();
